...
namespace FTAnalyzer
{
	public partial class FamilyTree
	{
		...
		{
			if (unknownIndividualFactTypes.Count > 0 && !GeneralSettings.Default.IgnoreFactTypeWarnings)
			{
				...
				foreach (string tag in unknownIndividualFactTypes.Keys)
				{
					int count = unknownIndividualFactTypes[tag].Count;
				...
				if (unknownFamilyFactTypes.Count > 0)
				{
					...
					{
						int count = unknownFamilyFactTypes[tag].Count;
						bool ignore = DatabaseHelper.IgnoreCustomFact(tag);
						if (count > 0 && !ignore)
							outputText.Report($"\nFound {count} Family facts of unknown/custom fact type {tag}");
					}
				}
				outputText.Report("\n");
			}
		...
		void AddCustomFacts(Individual individual)
		{
			foreach (string factType in unknownIndividualFactTypes.Keys)
			{
				if (individual.AllFacts.Any(x => x.FactTypeDescription == factType))
					unknownIndividualFactTypes[factType].Add(individual);
			}
		}
		...
		void AddCustomFacts(Family family)
		{
			foreach (string factType in unknownFamilyFactTypes.Keys)
			{
				if (family.Facts.Any(x => x.FactTypeDescription == factType))
					unknownFamilyFactTypes[factType].Add(family);
			}
		}
		...
		public IEnumerable<Individual> GetAllRelationsOfType(RelationshipType relationType) => individuals.Filter(ind => ind.RelationType == relationType);

		public IEnumerable<Individual> GetUncertifiedFacts(string factType, RelationshipType relationType)
		{
		...
		void ClearRelations()
		{
			foreach (Individual i in individuals)
			{
				i.RelationType = RelationshipType.UNKNOWN;
		...
		void AddDirectParentsToQueue(Individual indiv, Queue<Individual> queue, IProgress<string> outputText)
		{
			foreach (ParentalRelationship parents in indiv.FamiliesAsChild)
			{
				...
				if (family.Husband is not null && parents.IsNaturalFather && indiv.RelationType == RelationshipType.DIRECT)
				{
					BigInteger newAhnentafel = indiv.Ahnentafel * 2;
					if (family.Husband.RelationType != RelationshipType.UNKNOWN && family.Husband.Ahnentafel != newAhnentafel)
				...
				if (family.Wife is not null && parents.IsNaturalMother && indiv.RelationType == RelationshipType.DIRECT)
				{
					BigInteger newAhnentafel = indiv.Ahnentafel * 2 + 1;
					if (family.Wife.RelationType != RelationshipType.UNKNOWN && family.Wife.Ahnentafel != newAhnentafel)
		...
		static void AddParentsToQueue(Individual indiv, Queue<Individual> queue)
		{
			foreach (ParentalRelationship parents in indiv.FamiliesAsChild)
			{
				Family family = parents.Family;
				// add parents to queue
				if (family?.Husband?.RelationType == RelationshipType.UNKNOWN)
					queue.Enqueue(family.Husband);
				if (family?.Wife?.RelationType == RelationshipType.UNKNOWN)
		...
		//        {
		//            // add child to queue
		//            if (child.RelationType == RelationshipType.BLOOD || child.RelationType == RelationshipType.UNKNOWN)
		//            {
		//                child.RelationType = RelationshipType.BLOOD;
		...
		public void SetRelations(string startID, IProgress<string> outputText)
		{
			...
			ind.RelationType = RelationshipType.DIRECT;
			ind.Ahnentafel = 1;
			maxAhnentafel = 1;
			...
			IEnumerable<Individual> directs = GetAllRelationsOfType(RelationshipType.DIRECT);
			...
			while (queue.Count > 0)
			{
				...
				foreach (Family family in parentFamilies)
				{
					...
					family.SetSpouseRelation(ind, RelationshipType.MARRIEDTODB);
					// all children of direct ancestors and blood relations
					// are blood relations
					family.SetChildRelation(queue, RelationshipType.BLOOD);
			...
			IEnumerable<Individual> marriedDBs = GetAllRelationsOfType(RelationshipType.MARRIEDTODB);
			...
			while (queue.Count > 0)
			{
				...
				RelationshipType relationship = ind.RelationType;
				if (relationship == RelationshipType.MARRIAGE ||
					relationship == RelationshipType.MARRIEDTODB ||
					relationship == RelationshipType.UNKNOWN)
				{
					// set this individual to be related by marriage
					if (relationship == RelationshipType.UNKNOWN)
						ind.RelationType = RelationshipType.MARRIAGE;
					AddParentsToQueue(ind, queue);
					IEnumerable<Family> parentFamilies = ind.FamiliesAsSpouse;
					foreach (Family family in parentFamilies)
					{
						family.SetSpouseRelation(ind, RelationshipType.MARRIAGE);
						// children of relatives by marriage that we haven't previously 
						// identified are also relatives by marriage
						family.SetChildRelation(queue, RelationshipType.MARRIAGE);
					}
				}
			}
			...
			while (keepLooping)
			{
				...
				foreach (Family f in fams)
				{
					...
					{
						if (i.RelationType == RelationshipType.UNKNOWN)
						{
							i.RelationType = RelationshipType.LINKED;
		...
		{
			IEnumerable<Individual> directs = GetAllRelationsOfType(RelationshipType.DIRECT);
			IEnumerable<Individual> blood = GetAllRelationsOfType(RelationshipType.BLOOD);
			IEnumerable<Individual> married = GetAllRelationsOfType(RelationshipType.MARRIEDTODB);
		...
		public string PrintRelationCount()
		{
			...
			Dictionary<RelationshipType, int> relations = [];
			foreach (RelationshipType type in Enum.GetValues<RelationshipType>())
			{
				relations[type] = 0;
			}
			foreach (Individual i in individuals)
			...
			sb.Append($"Direct Ancestors: {relations[RelationshipType.DIRECT]}\n");
			sb.Append($"Descendants: {relations[RelationshipType.DESCENDANT]}\n");
			sb.Append($"Blood Relations: {relations[RelationshipType.BLOOD]}\n");
			sb.Append($"Married to Blood or Direct Relation: {relations[RelationshipType.MARRIEDTODB]}\n");
			sb.Append($"Related by Marriage: {relations[RelationshipType.MARRIAGE]}\n");
			sb.Append($"Linked through Marriages: {relations[RelationshipType.LINKED]}\n");
			sb.Append($"Unknown relation: {relations[RelationshipType.UNKNOWN]}\n");
			if (relations[RelationshipType.UNSET] > 0)
				sb.Append($"Failed to set relationship: {relations[RelationshipType.UNSET]}\n");
			sb.Append('\n');
			return sb.ToString();
		}
		
		public IEnumerable<Individual> DirectLineIndividuals => AllIndividuals.Filter(i => i.RelationType == RelationshipType.DIRECT || i.RelationType == RelationshipType.DESCENDANT);
	...
	public async static Task<List<DisplayFact>> AddWorldEvents(int earliestYear, DateTime chosenDate, bool wholeMonth, int stepSize, IProgress<int> progress)
	{
		...
		for (int year = earliestYear; year <= chosenDate.Year; year++)
		{
			...
			{
				URL = wholeMonth ?
						@"https://www.vizgr.org/historical-events/search.php?links=true&format=xml&begin_date=" + year.ToString() + chosenDate.ToString("MM", CultureInfo.InvariantCulture) + "00" +
